	...
	for dimension_data in dimensions:
		...
		if not frappe.db.exists("Inventory Dimension", dimension_name):
			try:
				...
				frappe.logger().error(f"Error creating Inventory Dimension '{dimension_name}': {e!s}")
				frappe.log_error(
					title=f"Error creating Inventory Dimension '{dimension_name}'",
...
def create_maintenance_fault_types():
	...
	for fault in fault_types:
		if not frappe.db.exists("Maintenance Fault Type", fault):
			try:
				doc = frappe.get_doc({"doctype": "Maintenance Fault Type", "fault_type": fault})
