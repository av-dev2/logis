...
class TripSettlement(Document):
	...
	def on_submit(self):
		...
		frappe.db.set_value("Trip Assignment", self.trip_assignment, "settled", 1)
	...
	def validate_duplicate_movement_order(self):
		...
		for row in self.trips:
			duplicates = (
				frappe.qb.from_(tsd)
				.select(tsd.name, tsd.parent)
				.where((tsd.movement_order == row.movement_order) & (tsd.name != row.name))
			).run(as_dict=True)
	...
	def create_stock_entry(self):
		...
		stock_entry = create_stock_entry(purpose="Material Issue", items=items, source_doc=self)
